...
resource "oci_identity_policy" "DataSafetoLoggingFunctionsPolicy" {
  ...
  statements = ["Allow dynamic-group ${oci_identity_dynamic_group.DataSafetoLoggingFunctionsServiceDynamicGroup[0].name} to use log-content in compartment id ${var.compartment_ocid}", 
